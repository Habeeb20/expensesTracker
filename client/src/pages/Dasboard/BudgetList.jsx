...
const BudgetManager = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [budgetsJson, analyticsJson, catsJson] = await Promise.all([budgetRes.json(), analRes.json(), catRes.json()]);
      if (budgetsJson.success && analyticsJson.success && catsJson.success) {
        setBudgets(budgetsJson.budgets);
        setAnalytics(analyticsJson.analytics);
        setCategories(catsJson.categories.filter(c => c.type === 'expense'));
      }
    } catch (err) {
      toast.error('Failed to load');
    } finally {
  ...
  const budgetBarData = [
    { name: 'Daily', budget: totalBudget },
    { name: 'Weekly', budget: totalBudget },
    { name: 'Monthly', budget: totalBudget },
    { name: 'Yearly', budget: totalBudget }
  ];
  ...
  return (
    <div className={cn('p-4 space-y-6', isDark ? 'bg-[#090909FF]' : 'bg-gray-50')}>
      ...
      <motion.div className={cn('p-6 rounded-2xl shadow-lg relative overflow-hidden', isDark ? 'bg-gray-900' : 'bg-white')}>
        <h3 className="text-lg font-semibold mb-4">Budget Spending</h3>
          ...
          <PieChart>
            ...
            >
              {pieData.map((_, i) => (
                <Cell
                  key={i}
                  fill={pieData[i].value === 1 && pieData[i].name === 'No data' ? '#94a3b8' : COLORS[i % COLORS.length]}
                />
              ))}
            </Pie>
            <Tooltip formatter={(v) => `₦${v.toLocaleString()}`} />
        ...
        {/* OPTIONAL: Spent vs Remaining text below */}
        {totalBudget > 0 && (
          <div className="mt-3 text-center space-y-1 text-sm">
            <p className="text-gray-600 dark:text-gray-400">
              <span className="font-medium">Spent:</span> ₦{totalSpent.toLocaleString()}
            </p>
            <p className="text-gray-600 dark:text-gray-400">
              <span className="font-medium">Remaining:</span> ₦{(totalBudget - totalSpent).toLocaleString()}
            </p>
          </div>
        )}
      </motion.div>
