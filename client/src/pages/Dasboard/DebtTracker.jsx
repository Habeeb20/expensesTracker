  ...
  return (
    <div className={cn('p-4 space-y-6', isDark ? 'bg-[#090909FF]' : 'bg-gray-50')}>
      <div className="max-w-5xl mx-auto">
        ...
        {/* MODAL FORM — NOW FULLY VISIBLE */}
        {showForm && (
          <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
            <div className={cn('rounded-3xl p-8 max-w-md w-full shadow-2xl', isDark ? 'bg-[#111111]' : 'bg-white')}>
              <h2 className="text-2xl font-bold mb-6">{editingId ? 'Edit Debt' : 'New Debt'}</h2>

              {/* Type */}
              <select
                value={form.type}
                onChange={(e) => setForm({ ...form, type: e.target.value })}
                className={cn('w-full p-4 rounded-xl mb-4', isDark ? 'bg-[#222222] text-white' : 'bg-gray-100')}
              >
                <option value="owe">I Owe Someone</option>
                <option value="owed">Someone Owes Me</option>
              </select>
              ...
              <input
                ...
                className={cn('w-full p-4 rounded-xl mb-6', isDark ? 'bg-[#222222] text-white' : 'bg-gray-100')}
              />

              {/* Buttons */}
              <div className="flex gap-4">
                <button onClick={resetForm} className="flex-1 py-4 bg-gray-600 text-white rounded-xl font-medium">
                  Cancel
                </button>
                <button onClick={saveDebt} className="flex-1 py-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-bold">
